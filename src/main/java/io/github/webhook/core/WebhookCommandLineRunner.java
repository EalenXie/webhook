...
public class WebhookCommandLineRunner implements CommandLineRunner {
    ...
    public void run(String... args) {
        ...
        if (!webhooks.isEmpty()) {
            ...
            sb.append("The following webhooks are available,Please fill in these address in your : \n");
            for (Webhook webhook : webhooks) {
                sb.append(String.format("Webhook Id:%s , Webhook address: %s%s/%s %n", webhook.getId(), contentPath, WebhookEndpoint.ENDPOINT_URL, webhook.getId()));
            }
